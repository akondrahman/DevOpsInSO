...
for(top_inex in 1:len_top_names+1)
{
  ...
  for(doc_index in 2:len_dic_topic)
  {
    ...
    if(tmp_==1)
    {
      ...
      #print(all_posts_data[doc_index, ])
      if(matched_row_accepted_ans_id !='')
      {
  ...
  print("***QNAA***")
  #qnaa_for_topic <- qcount_for_topic - qaa_for_topic
  print(qnaa_for_topic)
  qnaa_vector <- qnaa_vector[!qnaa_vector %in% c(0)]
  #print(length(qnaa_vector))
  comm_confidence <- qaa_for_topic  / qcount_for_topic 
  comm_confusion  <- qnaa_for_topic / qcount_for_topic 
